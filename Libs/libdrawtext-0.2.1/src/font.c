...
struct dtx_glyphmap *dtx_load_glyphmap_stream(FILE *fp)
{
	...
	while(hdr_lines < 3) {
		...
		if(line[0] == '#') {
			...
			if((res = sscanf_s(line + 1, " size: %d\n", &ptsize)) == 1) {
				gmap->ptsize = ptsize;

			} else if((res = sscanf_s(line + 1, " advance: %f\n", &line_adv)) == 1) {
				gmap->line_advance = line_adv;

			} else if((res = sscanf_s(line + 1, " %d: %fx%f+%f+%f o:%f,%f adv:%f\n",
			...
			case 1:
				if(sscanf_s(line, "%d %d", &gmap->xsz, &gmap->ysz) != 2) {
...
int dtx_save_glyphmap(const char *fname, const struct dtx_glyphmap *gmap)
{
	...
	char buffer[100];
	if(!(fp = fopen_s(&fp, fname, "wb"))) 
	{
		strerror_s(buffer, 100, errno);
		fprintf(stderr, "%s: failed to open file: %s: %s\n", __FUNCTION__, fname, buffer);
		return -1;
	}
